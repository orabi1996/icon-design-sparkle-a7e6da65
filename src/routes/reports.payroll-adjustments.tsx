...
export const Route = createFileRoute("/reports/payroll-adjustments")({
  head: () => ({ meta: [{ title: "تقرير تعديلات وتسويات المسير | التقارير المالية" }] }),
  component: PayrollAdjustmentsReport,
});
...
const ADJUSTMENT_TYPES = ["الكل", "إضافة مكافأة تميز", "استقطاع تسوية إدارية", "تعديل بدل سكن بأثر رجعي", "تصحيح ساعات عمل إضافي", "تعديل خصم بصمة"];
...
function PayrollAdjustmentsReport() {
  ...
  const [sortCol, setSortCol] = useState<string>("adjustment_date");
  ...
  const adjustmentRows = useMemo(() => {
    const types = [
      { t: "إضافة مكافأة تميز", amt: 1200, r: "مكافأة أداء الربع الأول الاستثنائي" },
      { t: "تصحيح ساعات عمل إضافي", amt: 450, r: "تسوية ساعات إضافية لمشروع التطوير" },
      { t: "استقطاع تسوية إدارية", amt: -300, r: "تعديل فرق أيام مباشرة بعد إجازة" },
      { t: "تعديل بدل سكن بأثر رجعي", amt: 1500, r: "ترقية وتعديل العقد الوظيفي" },
      { t: "تعديل خصم بصمة", amt: 250, r: "قبول عذر نسيان بصمة خروج رسمياً" },
    ];
    ...
    return employees.map((emp, i) => {
      ...
      return {
        ...
        adjustment_type: adj.t,
        amount: adj.amt,
        target_payroll: "مسير مايو 2025",
        adjusted_by: "مدير الموارد البشرية",
        adjustment_date: `2025/05/${10 + (i % 15)}`,
        reason: adj.r,
        approval_status: "معتمد من الإدارة المالية",
      } as AdjustmentItem;
    });
  }, [employees]);
  ...
  const filtered = useMemo(() => {
    ...
    return adjustmentRows.filter((r) => {
      ...
      if (f.adjustmentType && f.adjustmentType !== "الكل" && r.adjustment_type !== f.adjustmentType) return false;
  ...
  }, [adjustmentRows, appliedFilters, filters, globalSearch, colFilters]);
  ...
  const totals = useMemo(() => {
    return filtered.reduce(
      (acc, r) => {
        if (r.amount > 0) acc.additions += r.amount;
        else acc.deductions += Math.abs(r.amount);
        return acc;
      },
      { additions: 0, deductions: 0, count: filtered.length }
    );
  }, [filtered]);
  ...
  const exportExcel = (ext: "xlsx" | "xls") => {
    const headers = [
      "الرقم الوظيفي", "اسم الموظف", "الفرع", "القسم", "نوع التعديل",
      "المبلغ", "المسير المستهدف", "المستخدم", "تاريخ التعديل", "سبب التعديل", "الحالة"
    ];
    const data = sorted.map((r) => [
      r.emp_no, r.employee_name, r.branch, r.department, r.adjustment_type,
      r.amount, r.target_payroll, r.adjusted_by, r.adjustment_date, r.reason, r.approval_status
    ]);
    ...
    XLSX.utils.book_append_sheet(wb, ws, "تعديلات المسير");
    XLSX.writeFile(wb, `تقرير-تعديلات-المسير.${ext}`);
  };
  ...
  const exportCsv = () => {
    ...
    const rowsText = sorted.map((r) =>
      `"${r.emp_no}","${r.employee_name}","${r.branch}","${r.department}","${r.adjustment_type}",${r.amount},"${r.target_payroll}","${r.adjusted_by}","${r.adjustment_date}","${r.reason}","${r.approval_status}"`
    ).join("\n");
    const blob = new Blob(["\uFEFF" + headers + "\n" + rowsText], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "تقرير-تعديلات-المسير.csv";
    a.click();
  };
    ...
    <AppShell>
      ...
      <div className="mb-3 flex items-center justify-between border-b border-slate-200 pb-2">
        <h1 className="text-[16px] font-extrabold text-[#004e82] flex items-center gap-2">
          <MaterialIcon name="edit_note" size={22} className="text-[#0070c0]" />
          تقرير تعديلات وتسويات المسير
        </h1>
        <div className="text-[11px] text-slate-400">التقارير / تقارير ماليات الموظفين / تعديل المسير</div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4" dir="rtl">
        <div className="rounded-xl border border-blue-200 bg-blue-50/60 p-3 shadow-xs">
          <div className="text-[11px] font-bold text-slate-500">إجمالي الإضافات والمكافآت</div>
          <div className="text-lg font-extrabold text-[#0070c0] font-mono mt-1">+{totals.additions.toLocaleString()} ريال</div>
        </div>
        <div className="rounded-xl border border-rose-200 bg-rose-50/60 p-3 shadow-xs">
          <div className="text-[11px] font-bold text-slate-500">إجمالي الاستقطاعات والتسويات</div>
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-3 shadow-xs">
          <div className="text-[11px] font-bold text-slate-500">صافي أثر التعديل على المسير</div>
          <div className="text-lg font-extrabold text-emerald-700 font-mono mt-1">{(totals.additions - totals.deductions).toLocaleString()} ريال</div>
        </div>
        <div className="rounded-xl border border-purple-200 bg-purple-50/60 p-3 shadow-xs">
          <div className="text-[11px] font-bold text-slate-500">عدد حركات التعديل</div>
          <div className="text-lg font-extrabold text-purple-700 font-mono mt-1">{totals.count} حركة</div>
        </div>
      </div>
      ...
      <div className="mb-4 rounded-xl border border-[#b4c7e7] bg-[#f0f6ff]/70 p-4 shadow-sm" dir="rtl">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-2 items-end">
          ...
          <label className="flex flex-col gap-0.5">
            <span className="text-[11px] font-bold text-slate-700 text-right">نوع التعديل</span>
            <select
              value={filters.adjustmentType}
              onChange={(e) => setFilters((p) => ({ ...p, adjustmentType: e.target.value }))}
              className={inputCls}
            >
        ...
        <table className="w-full border-collapse text-[11px]">
          <thead>
            <tr className="bg-[#004e82] text-white">
              ...
              <th onClick={() => handleSort("adjustment_type")} className="px-3 py-2 font-extrabold border-r border-[#00385e] text-right cursor-pointer select-none">نوع التعديل</th>
              ...
              <th onClick={() => handleSort("target_payroll")} className="px-2.5 py-2 font-extrabold border-r border-[#00385e] text-center cursor-pointer select-none">المسير المستهدف</th>
              <th onClick={() => handleSort("adjusted_by")} className="px-2.5 py-2 font-extrabold border-r border-[#00385e] text-right cursor-pointer select-none">المستخدم</th>
              <th onClick={() => handleSort("adjustment_date")} className="px-2.5 py-2 font-extrabold border-r border-[#00385e] text-center cursor-pointer select-none">تاريخ التعديل</th>
              <th onClick={() => handleSort("reason")} className="px-3 py-2 font-extrabold border-r border-[#00385e] text-right cursor-pointer select-none">المبرر والسبب</th>
              <th className="px-2.5 py-2 font-extrabold text-center">الاعتماد</th>
            </tr>

            {/* Filter row */}
            <tr className="bg-[#e8f1fb] border-b border-slate-300">
              {["emp_no", "employee_name", "branch", "department", "adjustment_type", "amount", "target_payroll", "adjusted_by", "adjustment_date", "reason"].map((k) => (
            ...
            {isLoading ? (
              ...
              paginatedRows.map((r, idx) => (
                ...
                >
                  ...
                  <td className="px-3 py-1.5 border-r border-slate-200 text-right font-bold text-slate-800">{r.adjustment_type}</td>
                  <td className={`px-3 py-1.5 border-r border-slate-200 text-center font-mono font-bold ${
                    r.amount >= 0 ? "text-emerald-700 bg-emerald-50/50" : "text-rose-700 bg-rose-50/50"
                  }`}>
                    {r.amount >= 0 ? `+${r.amount.toLocaleString()}` : r.amount.toLocaleString()} ريال
                  </td>
                  <td className="px-2.5 py-1.5 border-r border-slate-200 text-center text-slate-700 font-medium">{r.target_payroll}</td>
                  <td className="px-2.5 py-1.5 border-r border-slate-200 text-right text-slate-700">{r.adjusted_by}</td>
                  <td className="px-2.5 py-1.5 border-r border-slate-200 text-center font-mono text-slate-600">{r.adjustment_date}</td>
                  <td className="px-3 py-1.5 border-r border-slate-200 text-right text-slate-600 text-xs">{r.reason}</td>
